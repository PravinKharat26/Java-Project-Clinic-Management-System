   ...
   class mreport extends JFrame implements ActionListener
{
	JLabel l0,l1,l2;
	JButton b1,btnBack;
	JComboBox c1;
	...
	public mreport()
	{
		...
		l1=new JLabel(" Person Responsible");
           		...
           		try
		{
			...
			rs=st.executeQuery("select distinct(personresponsible) from maintainance");
                                      		if(rs==null)
			...
			while(rs.next()){
                                  	     	...
                                  	     	ids=rs.getString("personresponsible");
		...
		String cname,bdate;
		  try
		  {
		   PreparedStatement pst=con.prepareStatement("select * from maintainance where personresponsible='"+from+"' " );
	           rs=pst.executeQuery();
